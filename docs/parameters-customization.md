...
For I / II / III, find `asset-confidentiality`, `asset-integrity`, `asset-availability`, `asset-authorization` or `asset-authentication` in the `assets` section. Each has three `interpretations[]` entries. The guidance distinguishes limited effects handled through routine work (I), substantial but contained consequences (II), and severe harm or loss of essential control (III). Short, grouped examples cover devices, gateways, applications, embedded firmware, shared libraries, data and system functions. Assess dependencies, affected users and recovery options: a small component can have severe consequences through the products that rely on it. These protection-need levels are separate from SL-T and CRA product classes. The Type entry retains the form mapping: HW to Component, SW/firmware/information to Data, and capabilities to Function. Authentication concerns identity; authorization concerns permissions. Edit the `assets` section’s `intro` for the common decision rule.
