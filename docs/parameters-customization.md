...
For I / II / III, find `asset-confidentiality`, `asset-integrity`, `asset-availability`, `asset-authorization` or `asset-authentication` in the `assets` section. Each has three `interpretations[]` entries. The guidance distinguishes local effects handled through routine work (I), substantial disruption or sensitive disclosure requiring deliberate recovery (II), and severe harm or loss of essential control (III). Each property and level includes labelled examples for hardware (HW), software (SW), firmware, data and functions. The asset’s role and consequences determine the level; its type alone does not. The Type entry explains the existing form mapping: HW to Component, SW/firmware/information to Data, and capabilities to Function. Authentication concerns identity; authorization concerns permissions. Edit the `assets` section's `intro` for the common decision rule. These are proposed company criteria, not IEC security-level definitions.
