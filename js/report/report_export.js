...
(function () {
  ...
  async function generateReportPdf() {
    ...
    const riskLabel = (deLabel) =>
      ri && ri.mapRiskLabel ? ri.mapRiskLabel(deLabel, lang) : deLabel;
    const isHiCrit = (deLabel) =>
      ri && ri.isHighOrCritical
        ? ri.isHighOrCritical(deLabel)
        : deLabel === 'Hoch' || deLabel === 'Kritisch';
    const locRoot = (entry) => {
      if (!entry) return '-';
    ...
    if (!analysis) {
      if (typeof showToast === 'function')
        showToast(
          typeof t === 'function'
            ? t('toast.noAnalysis', lang)
            : 'Keine aktive Analyse ausgewählt.',
          'warning'
        );
      return;
    }
    ...
    if (!doc) {
      if (typeof showToast === 'function')
        showToast(
          typeof t === 'function'
            ? t('toast.pdfMissing', lang)
            : 'PDF-Erzeugung nicht verfügbar (jsPDF nicht geladen).',
          'error'
        );
      return;
    }
    ...
    const riskWithResidualSorted = [...riskWithResidual].sort(
      (a, b) => b._rrValueNum - a._rrValueNum
    );
    ...
    if (hasResidualData) {
      summaryRows.push([L.riskDistRr, L.distLine(rrDist)]);
    }

    pdf.addTable([L.metric, L.value], summaryRows, [55, pdf.pageW - pdf.margin * 2 - 55]);
    ...
    if (topOnlyHighCritical.length === 0) {
      ...
      pdf.addTable(
        ...
        topN.map((r) => [
          r.id || '-',
          locRoot(r),
          (r.rootRiskValue ?? '-').toString(),
          riskLabel(r._riskLabel || 'Unbekannt'),
        ]),
        [16, 100, 22, 30]
      );
    }
      ...
      if (topRRHighCritical.length === 0) {
        ...
        pdf.addTable(
          ...
          topRRN.map((r) => [
            r.id || '-',
            locRoot(r),
            r._rrValueNum >= 0 ? r._rrValueNum.toFixed(2) : '-',
            riskLabel(r._rrLabel || 'Unbekannt'),
          ]),
          [16, 100, 22, 30]
        );
      }
    ...
    if (dsList.length === 0) {
      ...
      pdf.addTable(
        ...
        dsList.map((ds) => [
          ds.id,
          (typeof getLocalizedField === 'function'
            ? getLocalizedField(ds, 'short', lang, { fallback: true })
            : ds.short || '-') || '-',
          (typeof getLocalizedField === 'function'
            ? getLocalizedField(ds, 'name', lang, { fallback: true })
            : ds.name || '-') || '-',
          (typeof getLocalizedField === 'function'
            ? getLocalizedField(ds, 'description', lang, { fallback: true })
            : ds.description || '-') || '-',
        ]),
    ...
    if (risks.length === 0) {
      ...
      pdf.addTableGrid(
        ...
        [32, 28, 14, 12, 20, 32, 30],
        {
          zebra: true,
          noWrapCols: [1, 2, 3, 4],
          headerFill: [250, 250, 250],
          headerTextColor: [20, 20, 20],
          zebraFill: [252, 252, 252],
    ...
    if (risks.length > 0) {
      ...
      for (let ti = 0; ti < sortedTrees.length; ti++) {
        ...
        let svgText = null;
        const treeDpi =
          typeof h.treeRasterDpi === 'function' ? h.treeRasterDpi(sortedTrees.length) : 180;

        try {
          const dotContent =
            typeof window.exportRiskAnalysisToDot === 'function'
              ? window.exportRiskAnalysisToDot(analysis, entry.id)
              : null;

          if (dotContent) {
            if (typeof showToast === 'function')
              showToast(L.toastTree + ' ' + (entry.id || ''), 'info');
        ...
        doc.setTextColor(0);
        doc.text(
          L.attackTree + ' ' + (entry.id || '') + ': ' + (locRoot(entry) || ''),
          treeMargin,
          treeMargin
        );
    ...
    if (secGoals.length === 0) {
      ...
      const sortedSG = [...secGoals].sort((a, b) =>
        (a.id || '').localeCompare(b.id || '', undefined, { numeric: true })
      );
    ...
    if (rrEntries.length > 0 && typeof computeResidualTreeMetrics === 'function') {
      ...
      rrOverviewSorted.forEach((rrEntry) => {
        if (!rrEntry) return;
        const base =
          (analysis.riskEntries || []).find((e) => String(e.uid) === String(rrEntry.uid)) ||
          rrEntry;
    ...
    if (!rrEntries || rrEntries.length === 0 || typeof rrIterateLeaves !== 'function') {
      ...
      rrEntries.forEach((rrEntry) => {
        ...
        const origEntry = (analysis.riskEntries || []).find(
          (e) => String(e.id) === String(rrEntry.id)
        );
        const rootName = h.sanitizePdfText(
          locRoot(rrEntry) ||
            locRoot(origEntry) ||
            (rrEntry.name || rrEntry.root || '').toString() ||
            '-'
        );
        ...
        rrIterateLeaves(rrEntry, (meta) => {
          ...
          const leafKey =
            meta?.leafKey !== undefined && meta?.leafKey !== null ? String(meta.leafKey) : '';
          ...
          const iNorm =
            oLeaf.i_norm !== undefined &&
            oLeaf.i_norm !== null &&
            String(oLeaf.i_norm).trim() !== ''
              ? oLeaf.i_norm
              : origEntry &&
                  origEntry.i_norm !== undefined &&
                  origEntry.i_norm !== null &&
                  String(origEntry.i_norm).trim() !== ''
                ? origEntry.i_norm
                : rrEntry.i_norm !== undefined &&
                    rrEntry.i_norm !== null &&
                    String(rrEntry.i_norm).trim() !== ''
                  ? rrEntry.i_norm
                  : leaf.i_norm !== undefined
                    ? leaf.i_norm
                    : '';
          ...
          const rrClass = isNaN(rrNum)
            ? riskLabel('Unbekannt')
            : riskLabel(h.riskClassFromValue(rrNum).label);
    ...
    if (risks.length > 0) {
      ...
      for (let ti = 0; ti < sortedTrees.length; ti++) {
        ...
        try {
          const rrDot =
            typeof window.exportResidualRiskToDot === 'function'
              ? window.exportResidualRiskToDot(analysis, entry.id)
              : null;

          if (rrDot) {
            if (typeof showToast === 'function')
              showToast(L.toastRrTree + ' ' + (entry.id || ''), 'info');
        ...
        doc.setTextColor(0);
        doc.text(
          L.residualTree + ' ' + (entry.id || '') + ': ' + (locRoot(entry) || ''),
          rrMargin,
          rrMargin
        );
        doc.setFont('helvetica', 'normal');
    ...
    if (typeof showToast === 'function')
      showToast(
        typeof t === 'function' ? t('toast.reportOk', lang) : 'PDF Report erstellt.',
        'success'
      );
  }
