...
(function () {
  ...
  function pdfBuilder(doc) {
    ...
    const addImpactMatrixTable = (assets, dsList, impactMatrix) => {
      ...
      const assetW = dsCount <= 5 ? 55 : dsCount <= 7 ? 45 : dsCount <= 9 ? 38 : 32;

      const dsW = (totalW - assetW) / dsCount;

      // Font sizes dynamic: the more columns, the smaller
      const headerFontSize = dsCount <= 5 ? 6.5 : dsCount <= 7 ? 5.8 : dsCount <= 9 ? 5.2 : 4.5;

      const cellFontSize = dsCount <= 5 ? 7 : dsCount <= 7 ? 6.2 : dsCount <= 9 ? 5.5 : 4.8;

      const assetFontSize = dsCount <= 5 ? 7 : dsCount <= 7 ? 6.5 : dsCount <= 9 ? 6 : 5.5;

      // Row heights dynamic
      const headerH = dsCount <= 5 ? 14 : dsCount <= 7 ? 12 : 10;

      const rowH = dsCount <= 5 ? 10 : dsCount <= 7 ? 8.5 : 7.5;

      const cellPad = dsCount <= 7 ? 1.2 : 0.8;

      // Max character length for asset label
      const assetMaxChars = dsCount <= 5 ? 45 : dsCount <= 7 ? 35 : dsCount <= 9 ? 28 : 22;
      ...
      const impactStyle = (val) => {
        ...
        if (v === '3')
          return {
            fill: [245, 183, 177],
            text: useShortLabels ? '3' : '3 High',
            textColor: [0, 0, 0],
          };
        if (v === '2')
          return {
            fill: [248, 231, 159],
            text: useShortLabels ? '2' : '2 Medium',
            textColor: [0, 0, 0],
          };
        if (v === '1')
          return {
            fill: [171, 235, 198],
            text: useShortLabels ? '1' : '1 Low',
            textColor: [0, 0, 0],
          };
      ...
      const drawRow = (asset) => {
        ...
        for (let i = 0; i < dsCount; i++) {
          ...
          doc.text(st.text, x0 + dsW / 2, y0 + rowH / 2 + 1, {
            align: 'center',
            baseline: 'middle',
          });
          doc.setTextColor(0);
