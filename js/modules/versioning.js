...
function renderHistoryTable(analysis) {
  ...
  const sorted = history.slice().sort((a, b) => {
    const [aMajor, aMinor] = String(a.version || '0.0')
      .split('.')
      .map((n) => parseInt(n, 10) || 0);
    const [bMajor, bMinor] = String(b.version || '0.0')
      .split('.')
      .map((n) => parseInt(n, 10) || 0);
...
window.revertToVersion = (analysisId, version) => {
  ...
  showConfirmation({
    ...
    messageHtml:
      typeof tf === 'function'
        ? tf('version.restore.message', {
            version: escapeHtml(version),
            comment: escapeHtml(entry.comment),
          })
    ...
    onConfirm: () => {
      ...
      showToast(
        typeof tf === 'function'
          ? tf('toast.versionRestored', { version })
          : `Erfolgreich zur Version ${version} zurückgekehrt.`,
        'success'
      );
      ...
      if (elSB)
        elSB.textContent =
          typeof tf === 'function'
            ? tf('status.versionRestored', { version })
            : `Version ${version} wiederhergestellt.`;
    },
  });
};
...
window.openVersionCommentModal = () => {
  ...
  if (currentVersionInModalEl) {
    currentVersionInModalEl.textContent =
      analysis.metadata && analysis.metadata.version ? analysis.metadata.version : '-';
  }
...
function createNewVersion(comment) {
  ...
  if (!comment || comment.trim() === '') {
    showToast(
      typeof t === 'function'
        ? t('toast.versionNeedComment')
        : 'Abgebrochen. Versionskommentar ist notwendig.',
      'info'
    );
    return;
  }
  ...
  const existing = new Set(
    analysis.history.map((h) => String(h.version || '').trim()).filter(Boolean)
  );

  const currentVersion = String(
    analysis.metadata && analysis.metadata.version ? analysis.metadata.version : '0.0'
  );
  ...
  showToast(
    typeof tf === 'function'
      ? tf('toast.versionCreated', { version: newVersion })
      : `Neue Version ${newVersion} erstellt.`,
    'success'
  );
  ...
  if (elStatusBar)
    elStatusBar.textContent =
      typeof tf === 'function'
        ? tf('status.versionCreated', { version: newVersion })
        : `Neue Version ${newVersion} erstellt.`;
...
if (closeVersionControlModalEl) {
  closeVersionControlModalEl.onclick = () => {
    if (versionControlModalEl) versionControlModalEl.style.display = 'none';
  };
}

if (closeVersionCommentModalEl) {
  closeVersionCommentModalEl.onclick = () => {
...
if (versionCommentFormEl) {
  versionCommentFormEl.onsubmit = (e) => {
    ...
    if (comment) {
      ...
      showToast(
        typeof t === 'function'
          ? t('toast.versionCommentRequired')
          : 'Bitte geben Sie einen Versionskommentar ein.',
        'warning'
      );
    }
  };
