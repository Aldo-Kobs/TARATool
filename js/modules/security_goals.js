      ...
      .map((e) => {
        ...
        const name =
          typeof getLocalizedField === 'function'
            ? getLocalizedField(
                { title: e.rootName || '', title_en: e.rootName_en || '' },
                'title'
              ) ||
              e.rootName ||
              ''
  ...
  function openModal(goal, analysis) {
    ...
    if (goal) {
      if (titleEl)
        titleEl.textContent =
          typeof tf === 'function'
            ? tf('sg.modal.edit', { id: goal.id })
            : `Security Ziel ${goal.id} bearbeiten`;
      ...
      if (titleEl)
        titleEl.textContent = typeof t === 'function' ? t('sg.modal.new') : 'Neues Security Ziel';
      form.reset();
  ...
  function saveGoal(e) {
    ...
    if (id) {
      ...
      if (!existing) {
        if (typeof showToast === 'function')
          showToast(
            typeof t === 'function' ? t('toast.sgNotFound') : 'Security Ziel nicht gefunden.',
            'error'
          );
        return;
      }
      ...
      if (typeof showToast === 'function')
        showToast(
          typeof tf === 'function'
            ? tf('toast.sgUpdated', { id })
            : `Security Ziel ${id} aktualisiert.`,
          'success'
        );
    } else {
      ...
      if (typeof showToast === 'function')
        showToast(
          typeof tf === 'function'
            ? tf('toast.sgCreated', { id: newId })
            : `Security Ziel ${newId} erstellt.`,
          'success'
        );
    }
      ...
      .map((g) => {
        ...
        const refLabels = refs
          .map((id) => {
            ...
            const name =
              r && typeof getLocalizedField === 'function'
                ? getLocalizedField(
                    { title: r.rootName || '', title_en: r.rootName_en || '' },
                    'title'
                  ) ||
                  r.rootName ||
                  ''
    ...
    showConfirmation({
      ...
      onConfirm: () => {
        ...
        if (typeof showToast === 'function')
          showToast(
            typeof tf === 'function'
              ? tf('toast.sgDeleted', { id })
              : `Security Ziel ${id} gelöscht.`,
            'success'
          );
      },
    });
