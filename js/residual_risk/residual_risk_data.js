...
(function () {
  ...
  function rrIterateLeaves(entry, cb) {
    if (entry && entry.treeV2) {
      ...
      const walk = (node, bNum, parts) => {
        nodeCounter++;
        const nodeTitle =
          typeof getLocalizedField === 'function' && node
    ...
    entry.branches.forEach((branch, bIdx) => {
      ...
      if (depth === 3) {
        ...
        const leaves = Array.isArray(branch.leaves)
          ? branch.leaves
          : Array.isArray(branch?.l3_node?.leaves)
            ? branch.l3_node.leaves
            : [];
        leaves.forEach((leaf, lIdx) => {
          const lNum = lIdx + 1;
          ...
          cb({
            ...
            leafKey,
            branch,
            node: branch && branch.l3_node ? branch.l3_node : null,
            bNum,
            nNum: 1,
            lNum,
          });
  ...
  window.syncResidualRiskFromRiskAnalysis = function (analysis, saveAfter = false) {
    ...
    analysis.riskEntries.forEach((riskEntry) => {
      ...
      if (!riskEntry.uid) {
        ...
        riskEntry.uid =
          typeof generateUID === 'function' ? generateUID('risk') : 'risk_' + Date.now();
        changed = true;
      }
  ...
  window.computeResidualTreeMetrics = function (analysis, riskUid) {
    ...
    const residual = window.getResidualEntryByUid
      ? window.getResidualEntryByUid(analysis, riskUid)
      : null;
    ...
    try {
      ...
      console.warn(
        '[computeResidualTreeMetrics] applyWorstCaseInheritance error for uid',
        riskUid,
        e
      );
    }
    ...
    if (kstu.k === null && kstu.s === null && kstu.t === null && kstu.u === null) {
      console.warn(
        '[computeResidualTreeMetrics] KSTU all null after inheritance for uid',
        riskUid,
        'clone.kstu:',
        JSON.stringify(clone.kstu),
        'clone.treeV2?.kstu:',
        JSON.stringify(clone.treeV2?.kstu),
        'clone has treeV2:',
        !!clone.treeV2,
        'clone has branches:',
        !!clone.branches
