...
(function () {
  ...
  function rrRenderResidualLeafRiskValueHTML(leaf) {
    ...
    return (
      `${_t('rr.residualR')} <b style="color:${meta.color}">${rrEscapeHtml(meta.display)}</b>` +
  ...
  function rrBuildLeafLabel(meta) {
    ...
    const leafText = leaf
      ? rrLoc(leaf, 'text') ||
        (leaf.name ? String(leaf.name) : leaf.label ? String(leaf.label) : '')
      : '';
    ...
    const bName = meta?.branch
      ? rrLoc(meta.branch, 'title') ||
        rrLoc(meta.branch, 'name') ||
        (meta.branch.name ? String(meta.branch.name) : '') ||
        (meta.branch.title ? String(meta.branch.title) : '')
      : '';

    const nName = meta?.node
      ? rrLoc(meta.node, 'title') ||
        rrLoc(meta.node, 'name') ||
        (meta.node.name ? String(meta.node.name) : '') ||
  ...
  function rrRenderRootOverview(entries, analysis) {
    ...
    withMetrics.forEach(({ entry, base, m }) => {
      ...
      const fillClass =
        typeof getRiskBgClass === 'function' ? getRiskBgClass(resScore) : 'risk-bg-unknown';
  ...
  window.editResidualRiskTree = function (riskUid) {
    ...
    if (!entry) {
      if (typeof showToast === 'function')
        showToast(
          typeof t === 'function' ? t('toast.treeNotFound') : 'Angriffsbaum nicht gefunden.',
          'error'
        );
      return;
    }
