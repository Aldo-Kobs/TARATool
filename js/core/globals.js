...
const _FALLBACK_PROBABILITY_CRITERIA = {
  ...
  S: {
    ...
    options: [
      { value: '0.5', text: '0,5 - IT-Netzwerk beim Kunden', text_en: '0.5 - Customer IT network' },
  ...
  T: {
    ...
    options: [
      { value: '0.5', text: '0,5 - < 1 Woche', text_en: '0.5 - < 1 week' },
  ...
  U: {
    ...
    options: [
      { value: '0.5', text: '0,5 - Groß', text_en: '0.5 - High' },
...
function syncGlobalsFromAssessmentConfig(cfg) {
  ...
  DEFAULT_DAMAGE_SCENARIOS = Object.freeze(
    JSON.parse(JSON.stringify(c.defaultDamageScenarios || _FALLBACK_DAMAGE_SCENARIOS)).map((ds) =>
      Object.freeze(ds)
    )
  );

  PROBABILITY_CRITERIA = JSON.parse(
    JSON.stringify(c.probabilityCriteria || _FALLBACK_PROBABILITY_CRITERIA)
  );
  ...
  IMPACT_LABELS = Object.freeze({
    ...(c.impactScale?.labels || { 'N/A': 'N/A', 1: 'Low', 2: 'Medium', 3: 'High' }),
  });

  IMPACT_CSS_CLASSES = Object.freeze({
    ...(c.impactScale?.cssClasses || {
      3: 'impact-high',
      2: 'impact-medium',
      1: 'impact-low',
      'N/A': 'impact-na',
  ...
  SEVERITY_LEVEL_FACTORS = Object.freeze({
    ...(c.severityLevelFactors || { 0: 0.0, 1: 0.3, 2: 0.6, 3: 1.0 }),
  });

  PROTECTION_LEVEL_WEIGHTS = Object.freeze({
    ...(c.protectionLevels?.weights || { I: 0.6, II: 0.8, III: 1.0 }),
  });

  PROTECTION_LEVEL_RANKING = Object.freeze({
    ...(c.protectionLevels?.ranking || { '-': 0, I: 1, II: 2, III: 3 }),
  });
...
syncGlobalsFromAssessmentConfig(
  typeof ASSESSMENT_CONFIG !== 'undefined' ? ASSESSMENT_CONFIG : null
);
